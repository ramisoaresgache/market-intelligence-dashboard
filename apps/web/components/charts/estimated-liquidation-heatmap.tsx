...
import { decayedExposure, liquidationZoneEnd, type DerivativesSample, type EstimatedLiquidationZone } from "../../lib/market/engine/estimated-liquidations";
...
export function EstimatedLiquidationHeatmap({ symbol, samples, candles, zones, observed, historyState }: Props) {
  ...
  return <section className="chart-card liquidation-map-card" id="estimated-liquidations">
    <div className="panel-heading liquidation-map-heading"><div><span className="section-kicker">MAPA DE LIQUIDACIONES ESTIMADAS · ECHARTS</span><h2 className="heading-with-help">Zonas donde podría concentrarse el riesgo de liquidación <InfoTooltip label="Mapa de liquidaciones estimadas" text="Estima niveles donde podrían concentrarse cierres forzosos usando precio, cambios de interés abierto y escenarios de apalancamiento. El color expresa concentración relativa: amarillo es mayor. No son posiciones individuales publicadas por los exchanges." /></h2><p>Bandas modeladas sobre velas históricas reales, con zoom, paneo, crosshair y navegador nativos.</p></div>
    ...
    <div className="model-disclaimer"><b>NIVELES ESTIMADOS</b><span>Las velas son datos históricos reales de Binance USD-M. Las bandas son un modelo propio y desaparecen después de que una vela alcanza su precio. No representan posiciones individuales publicadas por los exchanges.</span></div>
    <details className="map-explainer"><summary>Cómo leer el mapa y el navegador inferior</summary><div><p><b>Color:</b> violeta indica menor concentración relativa y amarillo, mayor concentración estimada. El importe es útil para comparar niveles dentro de esta captura, no como cantidad exacta que vaya a liquidarse.</p><p><b>Uso:</b> una concentración puede actuar como zona imán o posible soporte/resistencia, pero no predice por sí sola la dirección. Cuando el precio alcanza un nivel, la banda deja de proyectarse.</p><p><b>Navegador inferior:</b> sirve para seleccionar y desplazar el tramo temporal visible. Arrastrá sus extremos para ampliar o reducir el período sin cambiar la captura.</p></div></details>
  </section>;
}
...
function buildModel(samples: DerivativesSample[], historical: HistoricalCandle[], zones: EstimatedLiquidationZone[], observed: LiquidationEvent[], threshold: number, rangeHours: 4 | 12 | 24) {
  ...
  const bands = candidates.flatMap((zone) => { const exposure = decayedExposure(zone, end, halfLife); const intensity = Math.min(1, (exposure / ceiling) ** 0.55); const zoneEnd = liquidationZoneEnd(zone, candles, end); return intensity >= threshold ? [[zone.createdAt, zone.liquidationPrice, zoneEnd, intensity, exposure, zone.leverage, zone.side === "long" ? 0 : 1]] : []; });
