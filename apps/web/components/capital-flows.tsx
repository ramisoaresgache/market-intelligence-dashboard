...
import { exportSvgAsPng } from "./charts/chart-export";
...
function BalanceChart({ points }: { points: ExchangeHistoryPoint[] }) {
  ...
  return (
    <div className="exchange-svg-wrap">
      <button className="standalone-export" type="button" onClick={() => svgRef.current && void exportSvgAsPng(svgRef.current, "btc-exchange-balance.png")}>Export PNG</button>
      <svg ref={svgRef} viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} role="img" aria-label="Histórico de saldo BTC y precio">
...
function FlowChart({ points }: { points: ExchangeHistoryPoint[] }) {
  ...
  return (
    <div className="exchange-svg-wrap">
      <button className="standalone-export" type="button" onClick={() => svgRef.current && void exportSvgAsPng(svgRef.current, "btc-exchange-flows.png")}>Export PNG</button>
      <svg ref={svgRef} viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} role="img" aria-label="Entradas y salidas diarias de BTC con precio BTC">
        <g className="chart-grid-lines">
          {flowTicks.map((value) => {
