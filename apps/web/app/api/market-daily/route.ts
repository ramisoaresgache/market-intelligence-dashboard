...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ symbol, generatedAt: Date.now(), source, open, close, changeUsd, changePct: open > 0 ? (changeUsd / open) * 100 : 0, points, warnings }, { headers: { "Cache-Control": "public, s-maxage=45, stale-while-revalidate=90" } });
}
...
function normalizePoints(rows: string[][], start: number, end: number): Point[] { return rows.map((row) => ({ ts: Number(row[0]), price: Number(row[4]) })).filter((point) => Number.isFinite(point.ts) && Number.isFinite(point.price) && point.price > 0 && point.ts >= start && point.ts <= end).sort((a, b) => a.ts - b.ts); }
