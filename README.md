...
El heatmap combina las celdas históricas del libro con velas públicas de Binance USD-M de 5 minutos. Las velas se dibujan por encima para aportar contexto de precio; no alteran ni completan artificialmente la liquidez registrada. Los controles permiten seleccionar fuente y ventanas de 15 minutos, 1 hora o 4 horas.

La pantalla principal se divide en tres vistas internas para evitar una página excesivamente larga:

- `Mercado y mapas`: mapa estimado, profundidad agregada y heatmap del order book;
- `Liquidaciones observadas`: resumen central 1 h / 4 h / 12 h / 24 h y eventos en vivo;
- `Flujos y reservas`: BTC en exchanges, ETF spot y movimientos on-chain.
